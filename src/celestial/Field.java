...
/*
 * A field is an endless cloud of 
 */
package celestial;
...
public class Field extends Celestial implements Serializable {
    ...
    public Field(Universe universe, Term field, int seed, Vector3f location, Vector3f bounds) {
    ...
    protected void alive() {
        Ship host = universe.getPlayer().getActiveShip();
        /*try {
            if (pointInsideField(host.getPhysicsLocation())) {
                System.out.println(pointInsideField(host.getLocation()));
                ...
                for (int a = 0; a < zones.size(); a++) {
                    ...
                    if (dist < getBlockSize() * 0.5) {
                        ...
                        //rootNode.detachChild(localBlock);
